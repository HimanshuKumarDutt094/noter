...
export function ProjectDialog({
  ...
  const handleExcerptChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => setExcerpt(e.target.value),
    []
  );
  ...
  const handleSubmit = useCallback(async () => {
    ...
    // Basic client-side validation to match schema constraints
    if (trimmed.length > 120) {
      toast.error("Project name too long", {
        description: "Keep names under 120 characters.",
      });
      return;
    }
    if (excerpt.trim().length > 240) {
      toast.error("Excerpt too long", {
        description: "Keep the excerpt under 240 characters.",
      });
      return;
    }
